...
describe("mobile sheet gestures", () => {
  test("dismisses after a deliberate downward drag", () => {
    expect(
      shouldDismissSheet({ distance: 145, velocity: 0.2, sheetHeight: 760 }),
    ).toBeTrue();
  });

  test("dismisses a short, fast downward flick", () => {
    expect(
      shouldDismissSheet({ distance: 58, velocity: 0.7, sheetHeight: 760 }),
    ).toBeTrue();
  });

  test("snaps back after a short or upward gesture", () => {
    expect(
      shouldDismissSheet({ distance: 58, velocity: 0.12, sheetHeight: 760 }),
    ).toBeFalse();
    expect(
      shouldDismissSheet({ distance: -120, velocity: -0.8, sheetHeight: 760 }),
    ).toBeFalse();
    expect(
      shouldDismissSheet({ distance: 180, velocity: -0.3, sheetHeight: 760 }),
    ).toBeFalse();
  });
